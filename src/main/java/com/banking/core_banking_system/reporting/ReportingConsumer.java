package com.banking.core_banking_system.reporting;
 ...
 * logic nghiệp vụ thật (không đẩy dữ liệu vào kho báo cáo thật).
 */
@Component
public class ReportingConsumer {
  ...
  @KafkaListener(topics = "transaction-posted-topic", groupId = "reporting-group")
  public void onMessage(String payload) {
    JsonNode node = objectMapper.readTree(payload);
    String transactionId = node.path("transactionId").asString();
    BigDecimal amount = totalDebitAmount(node.path("entries"));

    log.info("[Reporting] Would send alert for transaction {}, amount {}", transactionId, amount);
  }
